...
interface ChatInputAreaProps {
  ...
  placeholder: string;
  // Called when bot replies, pass (reply: string) => void
...
const ChatInputArea = forwardRef<HTMLTextAreaElement, ChatInputAreaProps>(
  (
    ...
    // Handle form submit: call Gemini Agent API
    const handleSendMessage = async (e: React.FormEvent) => {
      ...
      try {
        // (You may want to display the user's message here as well)
        const reply = await getAgentReply(message.trim());
